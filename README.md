...
A Python toolkit for evaluating Bitcoin mining investments. Built to support financial modelling for utility-scale mining operations, specifically calibrated for the **Axxela Omerelu 10MW project** in Nigeria.
...
2. **`monte_carlo_mining.py`** *(coming soon)* — Runs Monte Carlo simulation over the mining P&L model, using historical distribution parameters (mean, volatility, correlation, halving regime shifts) to generate probability distributions for IRR, payback period, and cumulative cashflow.
...
This toolkit was built for the evaluation of a 10MW Bitcoin mining deployment at Omerelu, Nigeria:

- **~3,000 Bitmain S19XP miners** (140 TH/s, 21 J/TH each)
- **Power Purchase Agreement** at $0.0385/kWh
...
- **Model 2 (BTC Price-driven):** Models revenue from first principles using BTC price, difficulty, and block reward as separate dynamic variables *(recommended — more transparent and calibratable)*
